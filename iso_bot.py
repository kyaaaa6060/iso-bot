...
async def handler(event):
    try:
        # Hesaba düşen her mesajı loga yazar
...
async def main():
    ...
    # KANALLARI/GRUPLARI HAFIZAYA ALMA (Önemli Kısım)
    print("🔄 Tüm sohbetler ve gizli kanallar önbelleğe yükleniyor...")
    async for dialog in client.iter_dialogs():
        # Kaynak kanalın adını loga basıp doğrularız
        if dialog.id == SOURCE_CHAT_ID:
            print(f"🔍 KAYNAK KANAL BULUNDU: {dialog.name} (ID: {dialog.id})")
